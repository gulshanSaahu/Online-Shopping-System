...
try
{
	...
	int sno=0;
	  ...
	  ResultSet rs2 =st.executeQuery("select * from users inner join cart on cart.email='"+email+"' and cart.status='bill'");
	  while(rs2.next())
	  {
...
<h3>Product Details</h3>
  ...
  ResultSet rs1=st.executeQuery("select * from cart inner join product on cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs1.next())
  {
	  sno=sno+1;
...
<h3>Thank you for shopping(online) with us!!</h3> 
